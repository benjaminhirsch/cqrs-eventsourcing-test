...
final class QueryBusFactory
{
    public function __invoke(ContainerInterface $container): QueryBus
    {
        ...
        if(!isset($config[QueryBus::class])) {
            throw InvalidConfig::create(sprintf('Missing %s config key in  container config!', QueryBus::class));
        }

        return new \App\Infrastructure\QueryBus(new MessageBus([
